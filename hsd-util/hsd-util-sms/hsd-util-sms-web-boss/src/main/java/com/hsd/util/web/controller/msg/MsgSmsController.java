...
public class MsgSmsController extends BaseController {
    ...
    public Response phydel(@PathVariable("id") Long id) {
        ...
        try {
           ...
           MsgSmsDto dto = new MsgSmsDto(){{
            ...
            result.message = msgSmsService.deleteData(dto);
        } catch (Exception e) {
            result = Response.error(e.getMessage());
        }
        return result;
     ...
     * <p> 信息保存
     */
    @RequiresPermissions(value = {"msgSms:add", "msgSms:edit"}, logical = Logical.OR)
    @RequestMapping(method = {RequestMethod.POST,RequestMethod.PUT}, value = acPrefix + "save")
    @RfAccount2Bean
    @ALogOperation(type = "修改", desc = "短信推送")
    @ApiOperation(value = "信息保存")
    public Response save(@Validated @ModelAttribute MsgSmsDto dto, BindingResult bindingResult) {
        log.info("MsgSmsController save.........");
        Response result = new Response();
        try {
            ...
            if (bindingResult.hasErrors()) {
                ...
                result = msgSmsService.saveOrUpdateData(dto);
