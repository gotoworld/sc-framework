...
function show_comment(){
		krcommon.call("js(update_comm_ele(get(scene[get(xml.scene)].name),get(view.hlookat),get(view.vlookat)))");
   ...
   $(".commentBox").css(css);
   krcommon.call("set(layer[skin_layer].visible,false)");
}
...
function cancel_comment(){
    ...
    $(".commentBox").hide();
	krcommon.call("removehotspot(commname);removeplugin(commname_avatar);set(layer[skin_layer].visible,true);");
}
//更新sname,ath,atv
...
function get_comm(){
    //取得当前scene
	var s = krcommon.get("scene[get(xml.scene)].name");
    $.post(basePath+"/s/common.getComment",{pid:pid,sceneId:s},function(data){
	    var data = eval('('+data+')');
		if(data.status==1){
...
function switch_show_comment(status){
    ...
    if(is_show_comment===true){
		for(var i=0;i<comment_list.length;i++){
		  krcommon.call("set(hotspot[userComm_"+comment_list[i].id+"].visible,false);set(layer[skin_co_btn_ico].url,%SWFPATH%/skin/no_comment.png);");
		}
		is_show_comment = false;
		cancel_comment();
    }
	    ...
	    for(var i=0;i<comment_list.length;i++){
		  krcommon.call("set(hotspot[userComm_"+comment_list[i].id+"].visible,true);set(layer[skin_co_btn_ico].url,%SWFPATH%/skin/comment.png);");
		}
...
//更新点赞数
function doZan(){
	krcommon.call("set(layer[skin_zan_btn_ico].enabled,false)");
	if(getcookie('thumbsUpNum'+pid)){
		alert('12小时内不能重复点赞');
	}else{
	    $.post(basePath+"/s/common.thumbsUpNum",{pid:pid},function(data){
		    var data = eval('('+data+')');
		    if(data.status==1){
			    krcommon.call("set(layer[skin_zan_btn_tit].html,'赞("+data.count+")');");
...
//更新点赞数，浏览量
function initStatis(){
    krcommon.call("set(layer[skin_zan_btn_tit].html,赞("+like_num+"))");
	krcommon.call("set(layer[skin_visit_num].html,人气："+visit_num+")");
	if(allow_comment==0){
	    krcommon.call("set(layer[skin_co_btn_container].visible,false);");
	}
	if (bgm!="") {
		krcommon.call("set(layer[skin_bgm_btn_container].visible,true)");
	};	
	//krcommon.call("set(layer[designer_name].html,"+company_name+designer_name+")");
	//krcommon.call("set(layer[designer_btn_ico].url,"+company_logo+")");
...
$(document).ready(function(){  
    $("#usercomm").keyup(function(){  
		...
		krcommon.call("set(plugin[commname_txt].html,"+content+");");
        if(curLength>0){
		    $("#doComm").removeAttr('style');
		}
		else{
			krcommon.call("set(plugin[commname_txt].html,拖动头像到你要评论的位置);");
		    $("#doComm").css({cursor:"not-allowed",backgroundColor:"#aaaaaa"});
		} 
    });
	$("#doComm").click(function(){
	    ...
	    $.post(basePath+"/s/common.addComment",{pid:pid,sceneId:sname,ath:ath,atv:atv,content:$("#usercomm").val()},function(data){
	    	var data = eval('('+data+')');
		    if(data.status==1){
			    //todo
				cancel_comment();
	...
	if(getcookie('pvNum'+pid)){
		...
		$.post(basePath+"/s/common.pvNum",{pid:pid},function(data){
		    var data = eval('('+data+')');
		    if(data.status==1){
			    krcommon.call("set(layer[skin_visit_num].html,赞("+data.count+"))");
