package com.hsd.account.platfor.aspect;
...
public class ALogAspect {
    ...
    //注入Service用于把日志保存数据库
    @Autowired
    private IOrgLogOperationService sysplatforLogService;
    ...
    public void doAfter(JoinPoint joinPoint) throws Exception {
        ...
        //读取session中的员工
        OrgStaffDto platfor = JwtUtil.getSubject(OrgStaffDto.class);
        //请求的IP
        ...
        try {
            ...
            sysplatforLogService.info(logArr[0], logArr[1], "" + request.getSession().getAttribute(CommonConstant.SESSION_KEY_DOMAIN_CODE), JSON.toJSONString(object), platfor.getId(), platfor.getName(), ip);
            log.debug("=====前置通知结束=====");
        } catch (Exception e) {
            //记录本地异常日志
    ...
    public void doAfterThrowing(JoinPoint joinPoint, Throwable e) throws Exception {
        ...
        try {
            ...
            sysplatforLogService.info(logArr[0], logArr[1], "" + request.getSession().getAttribute(CommonConstant.SESSION_KEY_DOMAIN_CODE), e.getMessage(), platfor.getId(), platfor.getName(), ip);
        } catch (Exception ex) {
