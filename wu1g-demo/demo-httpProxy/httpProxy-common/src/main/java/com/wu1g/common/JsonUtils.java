...
public class JsonUtils {
	...
	public static String mapToJsonByPairKey(Map map, String nameKey,
		...
		while (it.hasNext()) {
			...
			if (ValidatorUtil.isEmpty(entry.getValue())) {
				continue;
			}
			json.append(objectToJsonByPairKey((String) entry.getKey(),
					entry.getValue(), nameKey, valueKey));
			json.append(",");
		}
		if (json.length() > 1) {
	...
	public static String beanToJsonByPairKey(Object bean, String nameKey,
		...
		if (props != null) {
			for (int i = 0; i < props.length; i++) {
				...
				json.append(objectToJsonByPairKey(
						stringToJson(props[i].getName()), value, nameKey,
						valueKey));
				json.append(",");
			}
			if (json.length() > 1) {
	...
	public static void printList4AC(List list, Map map, PrintWriter writer)
			throws Exception {
		Object bean;
		PropertyDescriptor[] props = null;
		// writer.println("[");
			...
			if (props != null) {
				...
				for (int j = 0; j < props.length; j++) {
					if (map.containsKey(props[j].getName())) {
						...
						json.append(":");
						json.append(objectToJson(props[j].getReadMethod()
								.invoke(bean)));
						json.append(",");
					}
				}
	...
	public static String listToJson4AC(List<?> list, Map<String, String> map)
			throws Exception {
		StringBuilder json = new StringBuilder();
		Object bean;
		PropertyDescriptor[] props = null;
...
//		// ----------转换
//		return (ConfigJsonBean) mapper.readValue( json, ConfigJsonBean.class);
//	}
//	/**
//	 * String转为BaseSiteBean
//	 * @param json
//	 * @return
//	 */
//	public static BaseSiteBean jsonToBaseSiteBean(String json) throws Exception {
//		// ----------转换
//		return (BaseSiteBean) new ObjectMapper().readValue( json, BaseSiteBean.class);
//	}
	/**
	 * 配置文件转为Map
	 * 
...
//		 ConfigJsonBean stu =jsonToBean2("{\"title\":{\"tag\":\"1\",\"pro\":\"pro1\"}}");
//		 System.out.println(stu.getTitle().getTag());
//		 System.out.println(stu.getTitle().getPro());
//		try {
//			Map map = jsonToMap2("{\"xx\":\"22\",\"xx2\":\"222\"}");
//			System.out.println(map.get("xx"));
//			System.out.println(map.get("xx2"));
//		} catch (Exception e) {
