...
public class RequestHandler implements Runnable {
    private static transient Logger log = LogManager.getLogger(RequestHandler.class);
    /**
     * 客户端socket对象
     */
...
//					&& linkURL.indexOf( "443" ) == -1 
                    && remoteHeader.get("content-type") != null && remoteHeader.get("content-type").contains("text/html")) {
...
//					// 从工厂获得Socket连接
//					remoteSocket = ssf.createSocket( netURL.getHost(), netURL.getPort() == -1 ? 443 : netURL.getPort() );
//				} catch (Exception e) {
//					e.printStackTrace();
//					return;
//				}
//				// request = "CONNECT " + linkURL + " HTTP/1.0";
//				request = "CONNECT " + ("".equals( netURL.getPath() ) ? "/" : netURL.getPath()) + " HTTP/1.1";
//			} else {
            // 获得请求主机和端口
...
//			}
    ...
    private void remoteToClient() {
        ...
        try {
            ...
            // --分析获取远程服务器返回的具体信息-
            executor.execute(() -> {
                analyzeRemoteInputStream(netURL,linkURL,fileName,remoteHeader, remoteInputStreamClone);
            });
            // -------------------------end----------------------------------------------------------
        } catch (Exception e) {
     ...
     * @param stream
     */
    private void analyzeRemoteInputStream(URL netURL,String linkURL,String fileName, HashMap<String, String> remoteHeader, ByteArrayOutputStream stream) {
        writeFile(netURL,linkURL,fileName,remoteHeader, stream);
//		printHtml( stream );
    }
     ...
     * @param stream
     */
    private void printHtml(URL netURL,String linkURL,String fileName,HashMap<String, String> remoteHeader, ByteArrayOutputStream stream) {
     ...
     * @param stream
     */
    public void writeFile(URL netURL,String linkURL,String fileName,HashMap<String, String> remoteHeader, ByteArrayOutputStream stream) {
        ...
        // 获取头信息 用于判断流是否压缩
        String content_encoding =remoteHeader!=null?remoteHeader.get("content-encoding"):null;
        // 写入文件
        if (content_encoding != null && content_encoding.indexOf("gzip") != -1) {
        ...
        try {
            ...
            String content_type = remoteHeader!=null?remoteHeader.get("content-type"):null;
