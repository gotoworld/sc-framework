...
import com.wu1g.proxy.socks.SSLRequestHandler;
...
public class SSLSocketServer {
	private static transient Logger	log				= LogManager.getLogger( SSLSocketServer.class );
	/** socket服务器 实例 */
...
//			server = (SSLServerSocket) factory.createServerSocket( port );
	...
	public void accept() {
		while (true) {
			try {
				executor.execute( new SSLRequestHandler( server.accept() ) );
			} catch (IOException e) {
	...
	public static void main(String[] args) {
		...
		SSLSocketServer proxy = new SSLSocketServer( listener_port );
		proxy.accept();
	}
